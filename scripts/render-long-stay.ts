...
export function longStaySchema(language: StayLanguage) {
  ...
  return JSON.stringify({
    '@context': 'https://schema.org', '@graph': [
      ...
      { '@type': 'Accommodation', '@id': `${ORIGIN}/#scaleastay-apartment`, name: 'ScaleaStay', description: copy.intro, numberOfBedrooms: 1, image: [APARTMENTS[0].images[0], ...photosFor(language).map(photo => photo.src)], amenityFeature: CONFIRMED_AMENITIES, address: { '@type': 'PostalAddress', streetAddress: 'Via Giuseppe Saragat 11', addressLocality: 'Scalea', addressRegion: 'Calabria', addressCountry: 'IT' }, identifier: { '@type': 'PropertyValue', propertyID: 'CIN', value: 'IT078138C2VN4E3MCD' } },
...
.stay-languages{display:flex;flex-wrap:wrap;gap:8px 18px;padding-bottom:14px}.stay-languages a{font-size:14px}.stay-languages [aria-current="page"]{font-weight:700;color:#4338ca;text-decoration:underline}
...
@media(max-width:760px){.stay-wrap{padding:0 20px}.stay-top .stay-wrap{padding:16px 20px}.stay-hero{grid-template-columns:1fr;padding:40px 0;gap:28px}.stay-hero img{height:300px;order:2}.stay-grid,.stay-offer{grid-template-columns:1fr;gap:20px}.stay-section{padding:44px 0}.stay-offer{padding:28px 22px}.stay-card{padding:24px}.stay-top nav{font-size:12px}.stay-cta{width:100%}}
.stay-photos{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:28px;max-width:800px}.stay-photos figure{margin:0}.stay-photos img{display:block;width:100%;height:auto;aspect-ratio:3/4;border-radius:24px}.stay-photos figcaption{margin-top:12px;font-weight:700}.stay-photo-section{scroll-margin-top:24px}
@media(max-width:540px){.stay-photos{grid-template-columns:1fr;gap:28px}}
</style>`;
...
<header class="stay-top"><div class="stay-wrap"><a class="stay-brand" href="${home}">Scalea<span>Stay</span></a><nav aria-label="${escape(ui.navigation)}"><a href="${apartment}">${escape(ui.apartment)} →</a></nav></div><div class="stay-wrap"><nav class="stay-languages" aria-label="${escape(ui.languageLabel)}">${LONG_STAY_PAGES.map(page => `<a href="${page.path}" lang="${page.language}" hreflang="${page.language}"${page.language === language ? ' aria-current="page"' : ''}>${escape(page.name)}</a>`).join('')}</nav></div></header>
<main>
...
<section class="stay-section stay-white"><div class="stay-wrap"><h2>${escape(ui.features)}</h2><div class="stay-grid">${copy.features.map(item=>`<article class="stay-card"><h3>${escape(item.title)}</h3><p>${escape(item.text)}</p></article>`).join('')}</div></div></section>
...
<section class="stay-section"><div class="stay-wrap"><p class="stay-eyebrow">Via Giuseppe Saragat 11 · Scalea</p><h2>${escape(ui.nearby)}</h2><div class="stay-grid">${copy.nearby.map(item=>`<article class="stay-card"><div class="stay-distance">${escape(item.distance)}</div><h3>${escape(item.name)}</h3><p>${escape(item.text)}</p></article>`).join('')}</div>${guides ? `<a class="stay-link" href="${guides.noCar}">${escape(guides.noCarLabel)} →</a>` : ''}</div></section>
