...
<section class="stay-section stay-white"><div class="stay-wrap"><h2>${escape(ui.features)}</h2><div class="stay-grid">${[copy.features[2], copy.features[1], copy.features[0]].map(item=>`<article class="stay-card"><h3>${escape(item.title)}</h3><p>${escape(item.text)}</p></article>`).join('')}</div></div></section>
